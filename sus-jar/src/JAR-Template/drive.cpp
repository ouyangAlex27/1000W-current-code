...
//Input curve Functions

float Pilons(float input, float t){
...
void Drive::control_baked_tank(){
  double lYcord = controller(primary).Axis3.value();
  double rYcord = controller(primary).Axis2.value();

  double lXCord;
  double rXCord;

  lYcord > 0 ? lXCord = abs(controller(primary).Axis4.value()) : lXCord = -abs(controller(primary).Axis4.value());
  rYcord > 0 ? rXCord = abs(controller(primary).Axis1.value()) : rXCord = -abs(controller(primary).Axis1.value());

  int l_stick = left_curve_function(lYcord + lXCord, 7);
  int r_stick = left_curve_function(rYcord + rXCord, 7);

  DriveL.spin(fwd, to_volt(l_stick), volt);
  DriveR.spin(fwd, to_volt(r_stick), volt);
}
